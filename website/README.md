...
`.github/workflows/pages.yml` deploys when a GitHub Release is published whose tag is `vMAJOR.MINOR.PATCH.website` (example: `v1.0.1.website`). Other releases are ignored. In the repository, set Settings → Pages → Source to **GitHub Actions**. Then open Settings → Environments → github-pages → Deployment branches and tags, and either allow all tags or add a tag rule for `v*.*.*.website`. The default rule only allows `main`, so website release tags are rejected until this is changed. Mainnet contract addresses and the deployment block are baked into the workflow; optional repository variables `MAINNET_RPC_URL` and `WALLETCONNECT_PROJECT_ID` override the public RPC and enable WalletConnect.
